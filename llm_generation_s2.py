...
def main(args):
    ...
    input_file_path = f"./s2_data/{args.subset}.json"
    user_queries = load_json(input_file_path)

    folder_path = f"./llm_generation_results/s2/{args.prompt_method}/{args.subset}/{args.llm_model}"
    ...
    initial_prompt = shopping_initial_prompt_mod if args.subset == "online_shopping" else reddit_initial_prompt_mod
    
    results = load_json(output_file_path)

    for user_query_data in tqdm(user_queries, desc="Processing queries", unit="query"):
        refined_user_query = user_query_data.get("refined_user_query", "")
        ac_tree = user_query_data.get("ac_tree", "")
        ...
        chat_messages.append({"role": "user", "content": initial_prompt.replace("[AC_TREE]", ac_tree).replace("[USER_QUERY]", refined_user_query)})
